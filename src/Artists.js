...
import {getArtists, getGenres} from './CatalogAPI'
...
export default function Artists(props) {
  ...
  React.useEffect(() => {
    ...
    getArtists(input, (message, severity) => {
        if(severity === 'success'){
          if(Array.isArray(message) && message.length < 1){
            setValues({...values, message: `There are no artists.`, severity: 'error', open: true, nested_title: nested_title})
            return;
          }
        ...
        <div className={classes.heroContent}>
          <Container maxWidth="sm">
            <Typography component="h1" variant="h2" align="center" color="textPrimary" gutterBottom>
              {`${values.nested_title} Artists`}
            </Typography>
            <Typography variant="h5" align="center" color="textSecondary" paragraph>
              This is a list of Artists featured in our catalog. Explore the list to discover their albums!
            </Typography>
              ...
              <Grid item key={index} xs={12} sm={6} md={4}>
                <Card className={classes.card}>
                  ...
                  <Button
                    ...
                    startIcon={<AlbumIcon />}
                >
                    <Link to={'/albums/'+item.name+'/artist'}>
                        {`Albums`}
                    </Link>
                </Button>
                  </CardActions>
